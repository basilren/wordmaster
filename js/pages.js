...
function renderWordbank(){
  ...
  var sorted=db.units.slice().sort(function(a,b){return b.name.localeCompare(a.name);});
  list.innerHTML=sorted.map(function(u,i){
...
function renderUnitRadioList(containerId,filterSents){
  var html='';
  var colors=['#667eea','#22c55e','#f59e0b','#ef4444','#8b5cf6','#ec4899'];
  // Sort units by name descending (5BU6 first)
  var sorted=db.units.slice().sort(function(a,b){return b.name.localeCompare(a.name);});
...
function getFirstUnitId(){
  // Return the latest unit (by name descending)
  var sorted=db.units.slice().sort(function(a,b){return b.name.localeCompare(a.name);});
